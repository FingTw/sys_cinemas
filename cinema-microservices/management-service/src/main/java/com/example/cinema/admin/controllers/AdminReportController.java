...
import com.example.cinema.admin.dto.RevenueReportRowDTO;
import com.example.cinema.admin.feign.BookingClient;
...
public class AdminReportController {
    ...
    private final com.example.cinema.admin.services.JasperReportCache jasperReportCache;
    ...
    public ResponseEntity<byte[]> exportRevenueReportPdf() {
        ...
        try {
            ...
            List<Map<String, ?>> reportData = jdbcTemplate.query(sql, (rs, rowNum) -> {
                Map<String, Object> map = new HashMap<>();
                map.put("itemName", rs.getString("movie_title"));
                map.put("ticketCount", rs.getLong("total_bookings"));
                map.put("revenue", rs.getBigDecimal("total_revenue"));
                return map;
            });

            // 1. Load compiled JRXML template from cache
            JasperReport jasperReport = jasperReportCache.getRevenueReport();
            if (jasperReport == null) {
                log.error("[CINEMA-ADMIN] JRXML report template not found in cache!");
                return ResponseEntity.internalServerError().build();
            }

            // 2. Parameters
            Map<String, Object> parameters = new HashMap<>();
            parameters.put("CinemaName", "SYS CINEMA - HỆ THỐNG RẠP CHIẾU PHIM");
            parameters.put("ReportTitle", "BÁO CÁO DOANH THU & HIỆU SUẤT PHIM CHIẾU");
            parameters.put("FromDate", new java.util.Date(System.currentTimeMillis() - 30L * 24 * 3600 * 1000)); // Demo from 30 days ago
            parameters.put("ToDate", new java.util.Date());
            parameters.put("AdminName", currentUser != null ? currentUser : "Hệ Thống");
            parameters.put("GeneratedAt", new java.util.Date());
    ...
    public ResponseEntity<byte[]> exportTicketPdf(@org.springframework.web.bind.annotation.PathVariable String id) {
        ...
        try {
            ...
            // 2. Load compiled template from cache
            JasperReport jasperReport = jasperReportCache.getTicketReport();
            if (jasperReport == null) {
                log.error("[CINEMA-ADMIN] JRXML ticket template not found in cache!");
                return ResponseEntity.internalServerError().build();
            }
    ...
    public ResponseEntity<byte[]> exportReceiptPdf(@org.springframework.web.bind.annotation.PathVariable String id) {
        ...
        try {
            ...
            // 2. Load compiled template from cache
            JasperReport jasperReport = jasperReportCache.getReceiptReport();
            if (jasperReport == null) {
                log.error("[CINEMA-ADMIN] JRXML receipt template not found in cache!");
                return ResponseEntity.internalServerError().build();
            }
