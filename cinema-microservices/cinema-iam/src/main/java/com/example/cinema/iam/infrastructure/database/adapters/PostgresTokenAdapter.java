...
import com.example.cinema.iam.infrastructure.database.repositories.SpringDataTokenRepository;
...
public class PostgresTokenAdapter implements TokenRepository {
    ...
    @Override
    public java.util.List<AuthToken> findAllByUserId(String userId) {
        return springDataTokenRepository.findAllByUserId(userId).stream()
            .map(entity -> AuthToken.builder()
                .id(entity.getId() != null ? entity.getId().toString() : null)
                .userId(entity.getUserId())
                .tokenJti(entity.getTokenJti())
                .tokenHash(entity.getTokenHash())
    ...
    public void deleteById(String id) {
        springDataTokenRepository.deleteById(UUID.fromString(id));
    }
}
