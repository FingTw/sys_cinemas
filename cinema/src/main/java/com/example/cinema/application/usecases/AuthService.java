...
import com.example.cinema.application.ports.out.AuthGatewayPort;
...
import com.example.cinema.application.ports.in.LoginUseCase;
...
public class AuthService implements LoginUseCase {
    ...
    public AuthService(AuthGatewayPort authGatewayPort, JwtTokenProvider jwtTokenProvider,
            ...
            CryptoPort cryptoPort, TokenRepositoryPort tokenRepositoryPort, RoleRepository roleRepository,
    ...
    public AuthResponse login(String username, String encryptedPassword) {
        ...
        if (user.getActiveToken() != null && !user.getActiveToken().isEmpty()) {
            try {
                ...
                tokenBlacklistPort.blacklistToken(oldToken, expiresAt);
                tokenCachePort.removeToken(oldToken);
            } catch (Exception e) {
        ...
        try {
            ...
            tokenCachePort.cacheToken(accessToken, user.getId(), 600000); // 10 minutes

            // CACHE user roles and permissions for fast authorization checks
            tokenCachePort.cacheUserRoles(user.getId(), String.join(",", roleNames));
            tokenCachePort.cacheUserPermissions(user.getId(), String.join(",", effectivePermissions));
        } catch (Exception e) {
    ...
    public void register(RegisterRequest request) {
        ...
        try {
            ...
            com.example.cinema.domain.entities.Role userRole = roleRepository.findByName("USER")
    ...
    public AuthResponse refreshToken(String refreshTokenJti) {
        ...
        userRepositoryPort.save(user);
        tokenCachePort.cacheToken(newAccessToken, user.getId(), 600000);
